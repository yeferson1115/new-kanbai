...
@section('page_title', 'Listado de Sub Categorías')
@section('content')
<div class="content-header row">
    ...
    <div class="content-header-right text-md-end col-md-3 col-12 d-md-block d-none">
        <div class="mb-1 breadcrumb-right">
            <div class="dropdown">
                <a href="{{ url('subcategories/create') }}" class="btn btn-success waves-effect waves-float waves-light"><i class="fa fa-folder-o" aria-hidden="true"></i> Nueva sub categoría</a>
                            ...
                            <table class="table" id="datatables" >
                                <thead class="table-light">
                                    <tr >
                                        ...
                                        <th class="sorting" tabindex="0" aria-controls="DataTables_Table_3" rowspan="1" colspan="1" style="width: 54.7969px;" aria-label="Email: activate to sort column ascending">Categoría padre</th>
                                ...
                                @foreach ($subcategories as $item)
                                    <tr class="odd row{{ $item->id }}">
                                        <td>
                                            <a  class="mb-1 btn btn-warning waves-effect waves-float waves-light" href="{{ url('subcategories', [$item->encode_id,'edit']) }}" title="Editar"><i data-feather='edit-3'></i> </a>
                                            <form method="POST" action="">

                                                <div class="form-group">
                                                    <button type="submit" data-token="{{ csrf_token() }}" data-attr="{{ url('subcategories',[$item->encode_id]) }}" class="btn btn-danger waves-effect waves-float waves-light delete-user" value="Delete user"><i data-feather='trash-2'></i></button>
                                                </div>
                                            </form>
                                        </td>
                                        ...
                                        <td><img style="max-height: 70px;" src="{{ asset('images/subcategories/'.$item->file.'') }}" alt="Imagen"></td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
...
<script>
    $('.delete-user').click(function(e){
        ...
        Swal.fire({
        title: 'Seguro que desea eliminar la sub categoría?',
        text: "Se borraran todos los registros asociados",
        icon: 'warning',
